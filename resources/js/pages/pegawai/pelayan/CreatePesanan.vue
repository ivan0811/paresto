...
export default {    
    ...
    async mounted(){
        ...
        const pesanan = this.getPesananStorage.detail                
        if(!!pesanan){
            this.detailPesanan = pesanan.map(item => item.id)                        
            this.counter.map(item => {                
                let pesanan_count = pesanan.filter(val => val.id == item.id)                
                if(pesanan_count.length > 0) return item.count = pesanan_count[0].count                                    
            })
        }                
    },
    methods: {
        ...
        submitHandler(){            
            ...
            no_meja: this.getPesananStorage.no_meja, 
            ...
            this.$router.push({name: 'confirmPesanan'})
        }
    },
    computed: {
        ...mapGetters([
